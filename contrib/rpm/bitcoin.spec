...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8922
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8923
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18922
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18923
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8922
	%{_sbindir}/semanage port -d -p tcp 8923
	%{_sbindir}/semanage port -d -p tcp 18922
	%{_sbindir}/semanage port -d -p tcp 18923
	for selinuxvariant in %{selinux_variants}; do
