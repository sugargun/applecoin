Applecoin Core version *0.15.1* is now available from:

  <https://download.applecoin.org/applecoin-0.15.1/>
...
A number of changes to the way Applecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 apples per kB (0.00001 APL). This is to prepare for dropping the minimum transaction fee to 0.1 apples per kB (0.0001 APL) in the next release.
